...
def write_command(f: TextIO, command: CodeCommand, instr: str, i: int,
                  jump_pairs: list[JumpPair], options: Options) -> None:
...
def run_optimizations(contents: str, optimize: bool) -> Program:
    ...
    if optimize:
        ...
        while i < len(contents):
            # [-] and [+] set a cell to 0
            if i < len(contents) - 2 and contents[i] == '[' and \
                    contents[i + 1] in ('+', '-') and contents[i + 2] == ']':
                commands.append('0')
                i += 3
